...
local function creatureSayCallback(cid, type, msg)
	...
	elseif msgcontains(msg, 'no') then
	 ...
	 if msgcontains(msg, 'present') or msgcontains(msg, 'present box') then
        npcHandler:say('Do you bring me a present??', cid)
        npcHandler.topic[cid] = 2331 -- Assigning a new topic ID for the Logbook Quest dialogue
        return true
		end
		
	 if npcHandler.topic[cid] == 2331 and msgcontains(msg, 'yes') then
        
            if Player(cid):removeItem(2331, 1) then
			
				 local plevel = Player(cid):getLevel()
			
				local item = Player(cid):addItem(2480)
				item:setItemLevel(math.random(plevel*0.8,plevel*2), true)
				item:unidentify()
                --Player(cid):addExperience(100, true)
                npcHandler:say("Excellent. Here, take this helmet it has no use for me anyways.", cid)
            else
                npcHandler:say("You don't have the present with you.", cid)
            end
