...
local function creatureSayCallback(cid, type, msg)
	...
	local player = Player(cid)
	  ...
	  if msgcontains(msg, 'banana') or msgcontains(msg, 'banan') then
        npcHandler:say('Have you got a banana for me?', cid)
	 ...
	 if npcHandler.topic[cid] == 1001 and msgcontains(msg, 'yes') then
        if Player(cid):getStorageValue(2676) ~= 1 then
            if Player(cid):removeItem(2676, 1) then
                Player(cid):addItem(2526, 1)
                Player(cid):setStorageValue(2676, 1)
                Player(cid):addExperience(100, true)
                npcHandler:say("Excellent. Here, take this studded shield as a reward.", cid)
            else
                npcHandler:say("You don't have the banana with you.", cid)
            end
        else
